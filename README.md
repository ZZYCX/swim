...
Windows 电脑微信小程序抢票自动化脚本。程序根据本机日期选择次日的小程序链接，在微信 `swim` 聊天中发送并点击链接，进入小程序后关闭须知弹窗并点击“提交订单”；若未进入订单页，会在限定时间内根据页面状态重试，进入订单页后单次点击最终“提交”。
...
- `--max-wait`：等待小程序页面及首次提交后重试阶段的单阶段最长秒数，默认 `20`
...
- 不绕过验证码或微信登录。
- 不自动支付。
- 不并发、不高频点击；首次提交后仅在 `--max-wait` 限定时间内按页面状态重试。
- 订单页最终“提交”只点击一次。

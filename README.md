...
Windows 电脑微信小程序抢票自动化脚本。程序根据本机日期选择次日的小程序链接，在微信 `swim` 聊天中发送并点击链接，进入小程序后关闭须知弹窗，单次点击“提交订单”，并在订单页单次点击最终“提交”。
...
11. 每 2 秒通过 UIA 检查订单页标志，确认页面切换后检测并单次点击右下角“提交”。
12. `scheduler.py` 记录正式进程启动时间、首条 `main.py` 日志相对目标时间的偏差和最终退出码。
